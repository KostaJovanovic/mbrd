...
| `rich`   | `note` | The formatted content when present, and then authoritative over `text` (which it flattens to). `{ font, size, valign, blocks: [{ tag, align, text, wash? }] }` — `tag` is `h1`/`h2`/`p`, `align` is `left`/`center`/`right`, `font` is an allowlist key (`sheet`/`sans`/`serif`/`mono`), `size` a multiplier clamped to 0.7–1.8, `valign` is `top`/`middle`/`bottom`. `wash` is the highlighter drawn over that one line — `amber`/`terracotta`/`olive`/`graphite` — and is the one field written only when it is set: an unmarked line has no `wash` key rather than a `wash` of "none", so an unmarked note's blocks are byte-for-byte what they were before markers existed. An unknown wash is dropped rather than repaired, since there is no nearest marker to round a stranger's name to. Normalised on the way in (`normalizeNoteRich`): unknown values fall back, and the flattened text is held to `NOTE_MAX`. Absent on a legacy note, which is parsed back from `text`. |
