...
from `patch-notes.md`; `save.bat` re-runs it on every save, so what is committed
follows the prose.

What matters when touching it:

- **`patch-notes.md` is the only place the prose is written, and it is the
  author's to write.** Nothing checks it and nothing generates it. Do not add
  releases, fill a version gap, or edit the prose.
